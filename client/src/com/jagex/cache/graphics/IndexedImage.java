...
public final class IndexedImage extends Raster {
	...
	public void draw(int x, int y) {
		...
		if (width > 0 && height > 0) {
			draw(height, Raster.raster, raster, destStep, destOffset, width, sourceOffset, palette, sourceStep);
		}
	}
	...
	private void draw(int height, int[] raster, byte[] image, int destStep, int destIndex, int width, int sourceIndex,
			int[] palette, int sourceStep) {
		int minX = -(width >> 2);
		width = -(width & 3);
