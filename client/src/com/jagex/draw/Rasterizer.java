	...
	public static int[] method371(int id) {
		...
		if (lowMemory) {
			...
			for (int i1 = 0; i1 < 4096; i1++) {
				int i2 = ai[i1] = palette[image.getPixel(i1)] & 0xf8f8ff;
				if (i2 == 0) {
			...
			if (image.getWidth() == 64) {
				for (int j1 = 0; j1 < 128; j1++) {
					for (int j2 = 0; j2 < 128; j2++) {
						ai[j2 + (j1 << 7)] = palette[image.getPixel((j2 >> 1) + (j1 >> 1 << 6))];
					}
				}
			} else {
				for (int k1 = 0; k1 < 16384; k1++) {
					ai[k1] = palette[image.getPixel(k1)];
				}
			}
