...
public final class MapRegion {
	...
	public static void placeObject(CollisionMap map, SceneGraph scene, int id, int x, int y, int z, int type,
			int orientation, int[][][] ai, int plane) {
		int aY = ai[z][x][y];
		int bY = ai[z][x + 1][y];
		int cY = ai[z][x + 1][y + 1];
		int dY = ai[z][x][y + 1];
		...
		if (type == 22) {
			...
			if (definition.isSolid()) {
				map.flagObject(x, y, definition.getWidth(), definition.getLength(), definition.isImpenetrable(),
						orientation);
			}
		} else if (type >= 12) {
			Renderable object;
			...
			scene.addWall(key, x, y, plane, anIntArray152[orientation], primary, config, secondary, meanY,
					anIntArray152[corner]);
			if (definition.isSolid()) {
				map.flagObject(x, y, orientation, type, definition.isImpenetrable());
			}
		} else if (type == 3) {
			...
			if (type == 4) {
				...
				scene.addWallDecoration(key, y, orientation * 512, plane, COSINE_VERTICES[orientation] * displacement,
						meanY, object, x, config, SINE_VERTICIES[orientation] * displacement,
						anIntArray152[orientation]);
			} else if (type == 6) {
				Renderable object;
				if (definition.getAnimation() == -1 && definition.getMorphisms() == null) {
	...
	public final void decodeConstructedLandscapes(byte[] data, CollisionMap[] maps, SceneGraph scene, int plane,
			int topLeftRegionX, int topLeftRegionY, int collisionPlane, int regionX, int regionY, int orientation) {
				...
				do {
					...
					if (objectPlane == plane && y >= regionY && y < regionY + 8 && x >= regionX && x < regionX + 8) {
						...
						int localX = topLeftRegionX
								+ TileUtils.getObjectXOffset(x & 7, y & 7, definition.getWidth(),
										definition.getLength(), orientation);
						int localY = topLeftRegionY
								+ TileUtils.getObjectYOffset(x & 7, y & 7, definition.getWidth(),
										definition.getLength(), orientation);
	...
	public final void decodeConstructedMapData(byte[] data, int plane, int topLeftRegionX, int topLeftRegionY,
			int tileZ, int minX, int minY, int rotation, CollisionMap[] maps) {
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
	...
	private final void decodeMapData(Buffer buffer, int x, int y, int z, int regionX, int regionY, int orientation) {
	...
	public final void decodeRegionMapData(byte[] data, int dX, int dY, int regionX, int regionY, CollisionMap[] maps) {
		for (int z = 0; z < 4; z++) {
			...
			for (int centreX = -5; centreX < width + 5; centreX++) {
				...
				if (centreX >= 1 && centreX < width - 1) {
					...
					for (int centreY = -5; centreY < length + 5; centreY++) {
						...
						if (centreY >= 1
								&& centreY < length - 1
								&& (!lowMemory || (tileFlags[0][centreX][centreY] & BRIDGE_TILE) != 0 || (tileFlags[z][centreX][centreY] & 0x10) == 0
							...
							if (underlay > 0 || overlay > 0) {
								...
								if (underlay > 0) {
									...
									underlayColour = toHsl(hue, saturation, luminance);
									...
									adjustedColour = toHsl(hue, saturation, luminance);
								}
								...
								if (z > 0) {
									...
									if (overlay > 0 && !Floor.floors[overlay - 1].shadowed()) {
										flag = false;
									}
								...
								if (overlay == 0) {
									scene.method279(z, centreX, centreY, 0, 0, -1, centreHeight, eastHeight,
											northEastHeight, northHeight, light(underlayColour, centreLight),
											light(underlayColour, eastLight), light(underlayColour, northEastLight),
											light(underlayColour, northLight), 0, 0, 0, 0, i22, 0);
								} else {
									...
									if (texture >= 0) {
										textureColour = Rasterizer.averageTextureColour(texture);
										overlayColour = -1;
									} else if (floor.getMinimapColour() == 0xff00ff) { // transparent
										textureColour = 0;
										overlayColour = -2;
										texture = -1;
									} else {
										overlayColour = toHsl(floor.getHue(), floor.getSaturation(),
												floor.getLuminance());
										textureColour = Rasterizer.anIntArray1482[checkedLight(floor.getColour(), 96)];
									}

									scene.method279(z, centreX, centreY, tileType, orientation, texture, centreHeight,
											eastHeight, northEastHeight, northHeight,
											light(underlayColour, centreLight), light(underlayColour, eastLight),
											light(underlayColour, northEastLight), light(underlayColour, northLight),
											checkedLight(overlayColour, centreLight),
											checkedLight(overlayColour, eastLight),
											checkedLight(overlayColour, northEastLight),
											checkedLight(overlayColour, northLight), i22, textureColour);
								}
							}
						}
					}
					...
					for (int x = 0; x <= width; x++) {
						if ((anIntArrayArrayArray135[z][x][y] & flag) != 0) {
							...
							if (l10 >= 8) {
								...
								SceneGraph.method277(plane, x * 128, l15, x * 128, l5 * 128 + 128, k14, currentY * 128,
										1);
						...
						if ((anIntArrayArrayArray135[z][x][y] & k2) != 0) {
							...
							if ((j6 - i5 + 1) * (i9 - k7 + 1) >= 4) {
								...
								SceneGraph.method277(plane, i5 * 128, j12, j6 * 128 + 128, i9 * 128 + 128, j12,
										k7 * 128, 4);
	...
	public final void method174(int regionX, int regionY, int dx, int dy) {
		for (int y = regionY; y <= regionY + dy; y++) {
			for (int x = regionX; x <= regionX + dx; x++) {
	...
	private final void method175(CollisionMap map, SceneGraph scene, int id, int x, int y, int z, int type,
			int orientation) {
		...
		if (type == 22) {
			...
			if (type == 4) {
				...
				scene.addWallDecoration(key, y, orientation * 512, z, 0, mean, object, x, config, 0,
						anIntArray152[orientation]);
			} else if (type == 5) {
				int displacement = 16;
				...
				scene.addWallDecoration(key, y, orientation * 512, z, COSINE_VERTICES[orientation] * displacement,
						mean, object, x, config, SINE_VERTICIES[orientation] * displacement, anIntArray152[orientation]);
			} else if (type == 6) {
				Renderable object;
				if (definition.getAnimation() == -1 && definition.getMorphisms() == null) {
					object = definition.modelAt(4, 0, centre, east, northEast, north, -1);
	 ...
	 * @return The colour.
	 */
	private final int toHsl(int hue, int saturation, int luminance) {
		if (luminance > 179) {
			saturation /= 2;
		}
		if (luminance > 192) {
